...
mse.shape.GEVRidge.mat <- mse.loc.GEVRidge.mat <- mse.scale.GEVRidge.mat <- matrix(NA, nrow = sim, ncol = iter.ridge)
mse.shape.GEVLasso.mat <- mse.loc.GEVLasso.mat <- mse.scale.GEVLasso.mat <- matrix(NA, nrow = sim, ncol = iter.lasso)

warning.GEVSpatial <- fail.GEVSpatial <- fail.schlather <- fail.GEVRidge <- fail.GEVLasso <- vector()

#store final results
mse.loc.GEVSpatial <- mse.scale.GEVSpatial <- mse.shape.GEVSpatial <- mse.rl10.GEVSpatial <- mse.rl20.GEVSpatial  <- mse.rl50.GEVSpatial <- mse.rl100.GEVSpatial <- time.GEVSpatial <- vector()
...
while(ss <= sim){
  ...
  #tmp <- tryCatch(fitmaxstab(data=data, locations, cov.mod=maxcov, loc.form=form.loc.1, scale.form=form.scale.1, shape.form=form.shape.1), error = function(e) fail.GEVSchlather[ss] = 1)
  ...
  print(paste0("Sim: ", ss))
  write.csv(data, paste0("GEV_Stationary_Data_",seed,"_",ss,".csv"))
  
  ss = ss + 1
}


#save.image('GEVFinalResults.RData')
...
write.csv(tmp, paste0("GEV_Stationary_Results_",seed,".csv"))


write.csv(loc.val, paste0("GEV_Stationary_Loc_",seed,".csv"))
write.csv(scale.val, paste0("GEV_Stationary_Scale_",seed,".csv"))
write.csv(shape.val, paste0("GEV_Stationary_Shape_",seed,".csv"))
write.csv(coordinates, paste0("GEV_Stationary_Coordinates_",seed,".csv"))
